...
import (
	...
	"dissect/internal/core/actor"
	"dissect/internal/core/runtime/sequence"
...
type M struct {
	id       int32                     // ID
	actor    *actor.Actuator           // Actuator is a pointer to actuator.Actuator
		...
		for {
			...
			default:
				...
				if g != nil {
					...
					proto.Unmarshal(result.buffer, entity)
					packet := resolver.ExecuteProto(entity)
					packetBuffer, _ := proto.Marshal(packet)
					base.sm.Submit(result.id, packetBuffer)
...
func (m *M) bindCPU() {
	if m.id != 0 {
		if m.actor.Pid() != 0 {
			...
			bindCPU(m.actor.Pid(), IdleCpus[0])
			bindCPU(m.actor.Pid(), IdleCpus[1])
			IdleCpus = IdleCpus[2:]
		}
	} else {
