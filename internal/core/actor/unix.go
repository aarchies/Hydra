...
func NewWorker(id int32) *Actuator {
...
func (c *Actuator) Wait() {
	c.cmd.Wait()
}

func (c *Actuator) Exit() {
...
func (c *Actuator) signal() {
	c.sigCh <- os.Interrupt
	c.cancel()
}

func (c *Actuator) Input(seq uint64, buffer []byte) {
	c.inCh <- seqCh{
		SeqId:  seq,
		Buffer: buffer,
	}
...
func (c *Actuator) Output() chan seqCh {
	return c.outCh
}

func (c *Actuator) Pid() int {
	if c.cmd != nil {
